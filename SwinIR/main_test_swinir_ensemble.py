...
def transform_predict(model, mode, img_lq):
    if mode == 0:
        # identity
        output = model(img_lq)
    elif mode == 1:
        img_lq = torch.flipud(img_lq)
        output = model(img_lq)
        output = torch.flipud(output)
    elif mode == 2:
        img_lq = torch.fliplr(img_lq)
        output = model(img_lq)
        output = torch.fliplr(output)
    elif mode == 3:
        # torch.rot90 要注意指定的旋轉維度
        img_lq = torch.rot90(img_lq, 1, [2,3])
        output = model(img_lq)
        output = torch.rot90(output, 3, [2,3])
    elif mode == 4:
        img_lq = torch.rot90(img_lq, 2, [2,3])
        output = model(img_lq)
        output = torch.rot90(output, 2, [2,3])
    elif mode == 5:
        img_lq = torch.rot90(img_lq, 3, [2,3])
        output = model(img_lq)
        output = torch.rot90(output, 1, [2,3])
    elif mode == 6:
        img_lq = torch.flipud(torch.rot90(img_lq, 1, [2,3]))
        output = model(img_lq)
        output = torch.rot90(torch.flipud(output), 3, [2,3])
    elif mode == 7:
        img_lq = torch.flipud(torch.rot90(img_lq, 3, [2,3]))
        output = model(img_lq)
        output = torch.rot90(torch.flipud(output), 1, [2,3])
...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        with torch.no_grad():
            ...
            #-----upscale-----
            """
            應該可以在這裡寫self-ensemble
            """
            
            print("image name ", imgname, " test ensemble!!")
            output = model(img_lq)
            ...
            for i in mode_list:
                print("mode: ", i)
                output_ens = transform_predict(model, i, img_lq)
                #print("ensemble mode", i, " output shape: ", output.shape)
                output = output + output_ens
            output = output / (len(mode_list)+1)
 

            #-----------------

            #--將上述padding移除回正確得原始大小---
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        img_gt = cv2.imread("/home/skchen/DL_CV/set14_GT/"+filename, cv2.IMREAD_COLOR).astype(np.float32) / 255.
