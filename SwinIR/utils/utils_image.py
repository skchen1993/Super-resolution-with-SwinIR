...
def augment_img_MoA(img_lr, img_hr, mode=0):
    '''Kai Zhang (github: https://github.com/cszn)
    '''
    print("mode: ", mode)
    probs = [1.0, 1.0]
    ...
    img_lr = single2tensor3(img_lr)
    img_hr = single2tensor3(img_hr)

    # CHW
    if mode == 0:
        return img_lr, img_hr
    elif mode == 1:
        return torch.flipud(torch.rot90(img_lr, 1, [1,2])), torch.flipud(torch.rot90(img_hr, 1, [1,2]))
    elif mode == 2:
        return torch.flipud(img_lr), torch.flipud(img_hr)
    elif mode == 3:
        return torch.rot90(img_lr, 3, [1,2]), torch.rot90(img_hr, 3, [1,2])
    elif mode == 4:
        return torch.flipud(torch.rot90(img_lr, 2, [1,2])), torch.flipud(torch.rot90(img_hr, 2, [1,2]))
    elif mode == 5:
        return torch.rot90(img_lr, 1, [1,2]), torch.rot90(img_hr, 1, [1,2])
    elif mode == 6:
        return torch.rot90(img_lr, 2, [1,2]), torch.rot90(img_hr, 2, [1,2])
    elif mode == 7:
        return torch.flipud(torch.rot90(img_lr, 3, [1,2])), torch.flipud(torch.rot90(img_hr, 3, [1,2]))
    elif mode == 8:
        #blend
        #print("blend, img_lr=", img_lr.shape, " ,img_hr= ",img_hr.shape)
        img_lr, img_hr = blend(
            img_lr.clone(), img_hr.clone(),
        ...
        # RGB
        #print("RGB, img_lr=", img_lr.shape, " ,img_hr= ",img_hr.shape)
        img_lr, img_hr = rgb(
            img_lr.clone(), img_hr.clone(),
            prob=prob
        )
        ...
        #mixup
        print("mixup, img_lr=", img_lr.shape, " ,img_hr= ",img_hr.shape)
        img_lr, img_hr = mixup(
            img_lr.clone(), img_hr.clone(),
            prob=prob, alpha=alpha,
        )
...
'''
additional augmentation method
他好像是以有batch的觀點去寫的，但SwinIR 寫在dataset的getitem, 沒有batch的概念阿~所以會有少一維的問題
所以涉及要去同一batch內拿其他圖片來混合用的都..應該無法
cutblur感覺也怪怪的@@ 
先用可以用ㄉXD
'''
def blend(im1, im2, prob=1.0, alpha=0.6):
    ...
    c = torch.empty((3, 1, 1), device=im2.device).uniform_(0, 255)
    rim2 = c.repeat((1, im2.size(1), im2.size(2)))
    rim1 = c.repeat((1, im1.size(1), im1.size(2)))
...
def _cutmix(im2, prob=1.0, alpha=1.0):
    ...
    print("i am cutmix, im2.shape:", im2.shape)
