...
def main():
    ...
    parser.add_argument('--scale', type=int, default=3, help='scale factor: 1, 2, 3, 4, 8')  # 1 for dn and jpeg car
    parser.add_argument('--noise', type=int, default=15, help='noise level: 15, 25, 50')
    parser.add_argument('--jpeg', type=int, default=40, help='scale factor: 10, 20, 30, 40')
      #在此設定validation dataset路徑
    parser.add_argument('--folder_lq', type=str, default="", help='input low-quality val image folder')
    parser.add_argument('--folder_gt', type=str, default="", help='input ground-truth val image folder')
    ...
    for epoch in range(500000):  # keep running, is current_step that matter
        ...
        for i, train_data in enumerate(train_loader):
            # train_data 就是 {'L': img_L, 'H': img_H, 'L_path': L_path, 'H_path': H_path} 從dataset_sr,py來的
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                psnr_y, psnr  = set5test.validate_set5(args, model)
                print("psnr_y: ", psnr_y, ", psnr: ", psnr, ", psnr_record:", psnr_record)

                wandb.log({"val_psnr_y": psnr_y})
                wandb.log({"val_psnr": psnr})

                # 改成用psnr來看 (先不要用psnr_y)
                if (psnr > psnr_record):
                    model.save_better_model(psnr, args, current_step )
                psnr_record = psnr
                message = "------------------validation at " + str(current_step) + " iter, PSNR = " + str(psnr) + "---------------------"
                logger.info(message)
